...
public class EmojiSyncService {
    ...
    private void fetchStandardEmojis(Map<String, String> map) {
        try {
            // Using a lighter version or a direct map would be faster, but parsing the standard DB is most robust
            // We use a curated list or a transforming proxy if possible, but here we just read the standard JSON
            // Source: https://raw.githubusercontent.com/iamcal/emoji-data/master/emoji.json
            // Note: This is a large file (10MB+). For a mod, we might want to cache this on disk or use a smaller derived list.
            // For now, we'll try to use a slightly more optimized list provided by a CDN or similar if available, 
            ...
            java.net.URL url = new java.net.URL("https://raw.githubusercontent.com/iamcal/emoji-data/master/emoji.json");
            java.net.HttpURLConnection conn = (java.net.HttpURLConnection) url.openConnection();
            conn.setConnectTimeout(10000);
            conn.setReadTimeout(10000);
            
            try (java.io.InputStreamReader reader = new java.io.InputStreamReader(conn.getInputStream(), java.nio.charset.StandardCharsets.UTF_8)) {
                ...
                if (root.isJsonArray()) {
                    for (com.google.gson.JsonElement el : root.getAsJsonArray()) {
                        ...
                        if (!obj.has("short_name") || !obj.has("unified")) continue;
                        
                        String shortName = obj.get("short_name").getAsString();
                        String hex = obj.get("unified").getAsString().toLowerCase();
                        
                        // Twemoji URL
                        String iconUrl = "https://cdn.jsdelivr.net/gh/twitter/twemoji@latest/assets/72x72/" + hex + ".png";
