...
public class ModernChatScreen extends Screen {
    ...
    protected void init() {
        ...
        this.input = new EditBox(font, panelX + 10, panelY + panelHeight - 20, panelWidth - 40, 16, Component.literal(""));
        ...
        this.commandSuggestions = new net.minecraft.client.gui.components.CommandSuggestions(this.minecraft, this, this.input, this.font, false, true, 0, 7, true, Integer.MIN_VALUE);
        this.commandSuggestions.setAllowSuggestions(true);
        this.commandSuggestions.updateCommandInfo();
        
        // Wizard input
        this.wizardInput = new EditBox(font, panelX + panelWidth/2 - 60, panelY + panelHeight/2, 120, 16, Component.literal(""));
    ...
    public void tick() {
        ...
        if (lastColon >= 0) {
           ...
           if (!query.contains(" ")) {
               ...
               // Limit to 7
               if (suggestions.size() > 7) suggestions = suggestions.subList(0, 7);
               if (suggestions.isEmpty()) isSuggesting = false;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        if (showWizard) {
            ...
            if (wizardScroll < 0) wizardScroll = 0;
        ...
        if (!val.isEmpty()) {
            if (val.startsWith("ticket:")) {
                ...
                if (names.length > 1) {
                    ...
                    ChatChannelManager.getInstance().getOrCreateChannel(channelId, "@" + val); // Display name as comma list
    ...
    private void sendMessage() {
        ...
        if (activeId.startsWith("ticket-")) {
            ...
            // Local echo
            ChatChannelManager.getInstance().addMessage(activeId, Component.literal("§7[You -> " + target + "] " + text));
        } else {
            // Check for commands
            if (text.startsWith("/")) {
    ...
    private void renderWizard(GuiGraphics gfx, int mouseX, int mouseY) {
         ...
         if (connection != null) {
             ...
             for (var p : players) {
                 ...
                 boolean isHovered = mouseX >= listX && mouseX <= listX + listW && mouseY >= itemY && mouseY <= itemY + 12;
                 ...
                 // Head rendering is complex without easy access to skin location here, strictly use text for now
    ...
    private void renderMessages(GuiGraphics gfx, int mouseX, int mouseY) {
         ...
         // Scissor test could be added here to prevent overflow
         ...
         if (messages.isEmpty()) {
             gfx.drawCenteredString(font, "§7No messages yet...", panelX + panelWidth/2, msgY + msgHeight/2, TEXT_MUTED);
             return;
         }
         ...
         for (int i = messages.size() - 1; i >= 0; i--) {
             if (y < msgY) break; // clipped top
             
             Component msg = messages.get(i);
             // Use our texture renderer logic
             
             y -= 12; 
             renderStyledMessage(gfx, msg.getString(), msgX, y);
         }
    }
    ...
    private void renderStyledMessage(GuiGraphics gfx, String text, int x, int y) {
        ...
        while (emojiMatcher.find()) {
            ...
            ResourceLocation emojiTexture = cz.voidium.client.media.EmojiManager.getInstance().getEmojiTexture(emojiName);
