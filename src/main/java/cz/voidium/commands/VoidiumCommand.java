...
public class VoidiumCommand {
    ...
    public static void setSkinRestorer(SkinRestorer restorer) {
        skinRestorer = restorer;
    }

    public static void setVoteManager(VoteManager manager) {
        voteManager = manager;
    }
    ...
    private static int showHelp(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource()
                .sendSuccess(() -> Component.literal("§e/voidium restart <minutes> §7- Force server restart"), false);
        context.getSource().sendSuccess(
                () -> Component.literal("§e/voidium announce <message> §7- Send announcement to all players"), false);
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§e/voidium votes pending [player] §7- Show pending votes"), false);
        context.getSource().sendSuccess(() -> Component.literal("§e/voidium votes clear §7- Clear all pending votes"),
                false);
        context.getSource().sendSuccess(() -> Component.literal("§e/voidium status §7- Show mod status (everyone)"),
                false);
        context.getSource().sendSuccess(
                () -> Component.literal("§e/voidium clear §7- Clear all entities (items, mobs, xp, arrows)"), false);
        context.getSource().sendSuccess(
                () -> Component.literal("§e/voidium clear items|mobs|xp|arrows §7- Clear specific type"), false);
        context.getSource().sendSuccess(
                () -> Component.literal("§e/voidium clear preview §7- Preview what would be cleared"), false);
        return 1;
    }
    ...
    private static int skinRefresh(CommandContext<CommandSourceStack> context) {
        ...
        if (scheduled) {
            context.getSource().sendSuccess(
                    () -> Component.literal("§8[§bVoidium§8] §aSkin refresh scheduled for §e" + targetName), false);
        } else {
    ...
    private static int reload(CommandContext<CommandSourceStack> context) {
        try {
            ...
            cz.voidium.ranks.RankManager.getInstance().start(context.getSource().getServer());

            // Reload Stats Manager
            cz.voidium.stats.StatsManager.getInstance().stop();
            cz.voidium.stats.StatsManager.getInstance().start(context.getSource().getServer());

            context.getSource().sendSuccess(
                    () -> Component.literal("§8[§bVoidium§8] §aConfiguration reloaded successfully!"), false);
        } catch (Exception e) {
            context.getSource()
                    .sendFailure(Component.literal("§8[§bVoidium§8] §cError loading configuration: " + e.getMessage()));
        }
        return 1;
    }
    ...
    private static int forceRestart(CommandContext<CommandSourceStack> context) {
        ...
        if (restartManager != null) {
            ...
            context.getSource().sendSuccess(
                    () -> Component.literal("§8[§bVoidium§8] §eForced restart scheduled in " + minutes + " minutes!"),
                    true);
        } else {
    ...
    private static int announce(CommandContext<CommandSourceStack> context) {
        ...
        if (announcementManager != null) {
            ...
            context.getSource()
                    .sendFailure(Component.literal("§8[§bVoidium§8] §cAnnouncementManager is not available!"));
        }
        return 1;
    }
    ...
    private static int status(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§7Server name: §e" + context.getSource().getServer().getMotd()), false);
        String voidiumVersion = net.neoforged.fml.ModList.get().getModContainerById("voidium")
                .map(c -> c.getModInfo().getVersion().toString())
                .orElse("unknown");
        context.getSource().sendSuccess(() -> Component.literal("§7Voidium: §e" + voidiumVersion), false);
        context.getSource().sendSuccess(
                () -> Component.literal("§7Mod count: §e" + net.neoforged.fml.ModList.get().size()), false);
        ...
        context.getSource().sendSuccess(() -> Component.literal("§7MSPT: §e" + String.format("%.2f", avgTickTime)),
                false);
        ...
        if (restartConfig.getRestartType() == RestartConfig.RestartType.FIXED_TIME) {
            ...
            context.getSource().sendSuccess(
                    () -> Component.literal("§7Interval: §e" + restartConfig.getIntervalHours() + " hours"), false);
        }
        ...
        context.getSource()
                .sendSuccess(() -> Component.literal(
                        "§7Announcements every: §e" + announcementConfig.getAnnouncementIntervalMinutes() + " minutes"),
                        false);
        context.getSource().sendSuccess(
                () -> Component.literal("§7Announcement count: §e" + announcementConfig.getAnnouncements().size()),
                false);
        return 1;
    }
    ...
    private static int players(CommandContext<CommandSourceStack> context) {
        net.minecraft.server.MinecraftServer server = context.getSource().getServer();
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §fOnline players (§e" + server.getPlayerCount() + "§f):"),
                false);
        server.getPlayerList().getPlayers().forEach(player -> {
            int ping = player.connection.latency();
            context.getSource().sendSuccess(
                    () -> Component.literal("§7- §e" + player.getName().getString() + " §7(ping: §e" + ping + "ms§7)"),
                    false);
        });
        return 1;
    }
    ...
    private static int memory(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§7Used: §e" + usedMemory + "MB §7/ §e" + maxMemory + "MB"), false);
        context.getSource().sendSuccess(() -> Component.literal("§7Free: §e" + freeMemory + "MB"), false);
        context.getSource().sendSuccess(
                () -> Component
                        .literal("§7Usage: §e" + String.format("%.1f", (double) usedMemory / maxMemory * 100) + "%"),
                false);
        return 1;
    }
    ...
    private static int cancel(CommandContext<CommandSourceStack> context) {
        if (restartManager != null) {
            ...
            context.getSource().sendSuccess(
                    () -> Component.literal("§8[§bVoidium§8] §aAll manual restarts have been cancelled!"), false);
        } else {
            context.getSource().sendFailure(Component.literal("§8[§bVoidium§8] §cRestartManager is not available!"));
        }
        return 1;
    ...
    private static int config(CommandContext<CommandSourceStack> context) {
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §fConfiguration is stored in: §econfig/voidium/"), false);
    ...
    private static int votesPending(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(() -> Component.literal("§8[§bVoidium§8] §fTotal pending votes: §e" + total),
                false);
        return 1;
    }
    ...
    private static int votesPendingPlayer(CommandContext<CommandSourceStack> context) {
        ...
        if (count == 0) {
            context.getSource().sendSuccess(
                    () -> Component.literal("§8[§bVoidium§8] §7Player §e" + playerName + "§7 has no pending votes"),
                    false);
        } else {
            context.getSource().sendSuccess(() -> Component.literal("§8[§bVoidium§8] §aPlayer §e" + playerName
                    + "§a has §e" + count + "§a pending vote" + (count > 1 ? "s" : "")), false);
        }
        return 1;
    }
    ...
    private static int votesClear(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component
                        .literal("§8[§bVoidium§8] §aCleared §e" + total + "§a pending vote" + (total != 1 ? "s" : "")),
                false);
        return 1;
    }
    ...
    private static int clearAll(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource()
                .sendSuccess(() -> Component.literal("§8[§bVoidium§8] §aCleared: §e" + result.items + "§a items, §e"
                        + result.mobs + "§a mobs, §e" + result.xpOrbs + "§a XP orbs, §e" + result.arrows + "§a arrows"),
                        true);
        return 1;
    }
    ...
    private static int clearItems(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §aCleared §e" + result.items + "§a dropped items"), true);
        return 1;
    }
    ...
    private static int clearMobs(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource()
                .sendSuccess(() -> Component.literal("§8[§bVoidium§8] §aCleared §e" + result.mobs + "§a mobs"), true);
        return 1;
    }
    ...
    private static int clearXp(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §aCleared §e" + result.xpOrbs + "§a XP orbs"), true);
        return 1;
    }
    ...
    private static int clearArrows(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §aCleared §e" + result.arrows + "§a arrows"), true);
        return 1;
    }
    ...
    private static int clearPreview(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource()
                .sendSuccess(() -> Component.literal("§8[§bVoidium§8] §fWould clear: §e" + result.items + "§f items, §e"
                        + result.mobs + "§f mobs, §e" + result.xpOrbs + "§f XP orbs, §e" + result.arrows + "§f arrows"),
                        false);
        context.getSource().sendSuccess(
                () -> Component.literal("§8[§bVoidium§8] §fTotal: §e" + result.total() + "§f entities"), false);
        return 1;
    }
}